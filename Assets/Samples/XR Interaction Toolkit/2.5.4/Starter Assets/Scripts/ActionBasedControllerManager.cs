...
namespace UnityEngine.XR.Interaction.Toolkit.Samples.StarterAssets
{
    ...
    public class ActionBasedControllerManager : MonoBehaviour
    {
        ...
        //Jason added this
        [SerializeField]
        [Tooltip("The referense to the action to start ray cast aiming mode for this controller")]
        InputActionReference m_rayCastModeActivate;

        [SerializeField]
        [Tooltip("The reference to the action to cancel the raycast aiming mode for this controller.")]
        InputActionReference m_rayCastModeCancel;
        ...
        [SerializeField]
        [Tooltip("the reference for a grab")]
        InputActionReference m_grab;
        ...
        // For our input mediation, we are enforcing a few LDRules between direct, ray, and teleportation interaction:
        ...
        void SetupInteractorEvents()
        {
            ...
            if(m_rayCastModeActivate != null && m_rayCastModeCancel != null)
            {
                var raycastModeAction = GetInputAction(m_rayCastModeActivate);
                var cancelRayCastModeAction = GetInputAction(m_rayCastModeCancel);

                var grabAction = GetInputAction(m_grab);

                raycastModeAction.performed += StartRaycast;
                raycastModeAction.canceled += StopRayCast;
                grabAction.canceled += StopRayCast;
            }
        ...
        private void StopRayCast(InputAction.CallbackContext obj)
        {
            if (!m_grab.action.IsPressed())
            {
                m_RayInteractor.gameObject.SetActive(false);
            }
        }

        private void StartRaycast(InputAction.CallbackContext obj)
        {
            if (m_RayInteractor != null)
